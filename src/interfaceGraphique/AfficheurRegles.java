...
public class AfficheurRegles extends javax.swing.JPanel { //step 5 the third panel
    ...
    private RegleAssociation [] m_tReglesFiltrees = null;
    ...
    public AfficheurRegles(ContexteResolution contexteResolution) {
        ...
        //title font
        m_fontEnTete = UtilDessin.ChargerFonte("font_tahomabd.ttf");
        ...
        //item font
        m_fontItems = UtilDessin.ChargerFonte("font_timesbi.ttf");
        if (m_fontItems != null)
        ...
        //details font
        m_fontDetails = UtilDessin.ChargerFonte("arial.ttf");
    ...
    public void DefinirListeRegles(RegleAssociation [] tReglesFiltrees) {
    ...
    public float PeindreRegle(int iNumeroRegle, RegleAssociation regle, int iY, int iLargeurZone, Graphics2D g2D) {
        ...
        for (iIndiceCoteRegle = 0; iIndiceCoteRegle < 2; iIndiceCoteRegle++) {
            ...
            for (iIndiceItem = 0; iIndiceItem < iNombreItems; iIndiceItem++) {
                ...
                if (item.m_iTypeItem == Item.ITEM_TYPE_QUALITATIF) {
                    ...
                    fHauteurCumuleeElement += UtilDessin.PeindreTexteLimite(item.toString(), iLargeurMaxi, iPositionTexteX, iY+(int)fHauteurCumuleeElement, g2D);
            ...
            if (iNombreItemsQuantitatifs > 0) {
                ...
                for (iIndiceDisjonction = 0; iIndiceDisjonction < iNombreDisjonctions; iIndiceDisjonction++) {
                    ...
                    if (iIndiceDisjonction>0)
                    	fHauteurCumuleeElement += 30.0f + UtilDessin.PeindreTexteCentreLimite("OR", iLargeurMaxi-40, iPositionTexteX+20, iY+(int)fHauteurCumuleeElement+15, g2D);
                    ...
                    for (iIndiceItem = 0; iIndiceItem < iNombreItems; iIndiceItem++) {
                        ...
                        if (item.m_iTypeItem == Item.ITEM_TYPE_QUANTITATIF) {
                            if (bPremierItemInscrit)
                                fHauteurCumuleeElement += 15.0f;                            
                            fHauteurCumuleeElement += UtilDessin.PeindreTexteLimite(((ItemQuantitatif)item).toString(iIndiceDisjonction), iLargeurMaxItemsQuantX, iPositionItemsQuantX, iY+(int)fHauteurCumuleeElement, g2D);
                            bPremierItemInscrit = true;
                        }
                    }               
                    ...
                    if (iNombreDisjonctions > 1) {
                        ...
                        UtilDessin.PeindreAccolade(iPositionTexteX+10, (float)iY + fMilieuItems, (fHauteurCumuleeElement-fHauteurHautAccolade)/2 + 2.0f, true, g2D);
                        UtilDessin.PeindreAccolade(iPositionTexteX+10+iLargeurMaxi-20, (float)iY + fMilieuItems, (fHauteurCumuleeElement-fHauteurHautAccolade)/2 + 2.0f, false, g2D);
        ...
        UtilDessin.PeindreFleche(iLargeurMaxiGauche+35, iY + (int)fMilieuItems, g2D);
        
        // Display des accolades :
        ancienPinceau = g2D.getStroke();
        g2D.setStroke( new BasicStroke(2.0f) );

        UtilDessin.PeindreAccolade(10.0f, (float)iY + fMilieuItems, fMaxHauteur/2 + 2.0f, true, g2D);
        UtilDessin.PeindreAccolade(iLargeurMaxiGauche+30.0f, (float)iY + fMilieuItems, fMaxHauteur/2 + 2.0f, false, g2D);
        UtilDessin.PeindreAccolade(iLargeurMaxiGauche+110.0f, (float)iY + fMilieuItems, fMaxHauteur/2 + 2.0f, true, g2D);
        UtilDessin.PeindreAccolade(iLargeurMaxiGauche+iLargeurMaxiDroite+130.0f, (float)iY + fMilieuItems, fMaxHauteur/2 + 2.0f, false, g2D);
                ...
                for (iIndiceDisjonction = 0; iIndiceDisjonction < iNombreDisjonctions; iIndiceDisjonction++) {
                    ...
                    for (iIndiceItem = 0; iIndiceItem < iNombreItems; iIndiceItem++) {
                        ...
                        if (item.m_iTypeItem == Item.ITEM_TYPE_QUANTITATIF) {
                            ...
                            UtilDessin.PeindreTexteLimite(itemQuant.m_attributQuant.ObtenirNom()+" :", 90, 30, iY+(int)fHauteurCumulee, g2D);
                    ...
                    ancienPinceau = g2D.getStroke();
                    g2D.setStroke( new BasicStroke(2.0f) );
                    UtilDessin.PeindreAccolade(20, (float)iY+fHauteurHautAccolade+(fHauteurCumulee-10.0f-fHauteurHautAccolade)/2, (fHauteurCumulee-10.0f-fHauteurHautAccolade)/2 + 2.0f, true, g2D);
                    g2D.setStroke(ancienPinceau);
                }
        }        
        ...
        UtilDessin.PeindreTexteLimite("SUPPORTS :", 100, 10, iY+(int)fHauteurCumuleeGauche, g2D);
        fHauteurCumuleeGauche += 20.0f;        
        
        UtilDessin.PeindreTexteLimite("A and B", 80, 10, iY+(int)fHauteurCumuleeGauche, g2D);
        g2D.fill3DRect(100, iY+(int)fHauteurCumuleeGauche, (120*regle.m_iOccurrences) / iNombreLignesBD, 10, true);
        UtilDessin.PeindreTexteLimite(m_contexteResolution.EcrireSupport(regle.m_iOccurrences), 90, 230, iY+(int)fHauteurCumuleeGauche, g2D);
        fHauteurCumuleeGauche += 15.0f; 
        
        UtilDessin.PeindreTexteLimite("A", 80, 10, iY+(int)fHauteurCumuleeGauche, g2D);
        g2D.fill3DRect(100, iY+(int)fHauteurCumuleeGauche, (120*regle.m_iOccurrencesGauche) / iNombreLignesBD, 10, true);
        UtilDessin.PeindreTexteLimite(m_contexteResolution.EcrireSupport(regle.m_iOccurrencesGauche), 90, 230, iY+(int)fHauteurCumuleeGauche, g2D);
        fHauteurCumuleeGauche += 15.0f;        

        UtilDessin.PeindreTexteLimite("B", 80, 10, iY+(int)fHauteurCumuleeGauche, g2D);
        g2D.fill3DRect(100, iY+(int)fHauteurCumuleeGauche, (120*regle.m_iOccurrencesDroite) / iNombreLignesBD, 10, true);
        UtilDessin.PeindreTexteLimite(m_contexteResolution.EcrireSupport(regle.m_iOccurrencesDroite), 90, 230, iY+(int)fHauteurCumuleeGauche, g2D);
        fHauteurCumuleeGauche += 15.0f;        
        
        UtilDessin.PeindreTexteLimite("A and (~B)", 80, 10, iY+(int)fHauteurCumuleeGauche, g2D);
        g2D.fill3DRect(100, iY+(int)fHauteurCumuleeGauche, (120*regle.m_iOccurrences_Gauche_NonDroite) / iNombreLignesBD, 10, true);
        UtilDessin.PeindreTexteLimite(m_contexteResolution.EcrireSupport(regle.m_iOccurrences_Gauche_NonDroite), 90, 230, iY+(int)fHauteurCumuleeGauche, g2D);
        fHauteurCumuleeGauche += 15.0f; 
        
        UtilDessin.PeindreTexteLimite("(~A) and B", 80, 10, iY+(int)fHauteurCumuleeGauche, g2D);
        g2D.fill3DRect(100, iY+(int)fHauteurCumuleeGauche, (120*regle.m_iOccurrences_NonGauche_Droite) / iNombreLignesBD, 10, true);
        UtilDessin.PeindreTexteLimite(m_contexteResolution.EcrireSupport(regle.m_iOccurrences_NonGauche_Droite), 90, 230, iY+(int)fHauteurCumuleeGauche, g2D);
        fHauteurCumuleeGauche += 15.0f; 
        
        UtilDessin.PeindreTexteLimite("(~A) and (~B)", 80, 10, iY+(int)fHauteurCumuleeGauche, g2D);
        g2D.fill3DRect(100, iY+(int)fHauteurCumuleeGauche, (120*regle.m_iOccurrences_NonGauche_NonDroite) / iNombreLignesBD, 10, true);
        UtilDessin.PeindreTexteLimite(m_contexteResolution.EcrireSupport(regle.m_iOccurrences_NonGauche_NonDroite), 90, 230, iY+(int)fHauteurCumuleeGauche, g2D);
        fHauteurCumuleeGauche += 15.0f; 
        
        
        // Display the confidences :
        g2D.setColor( Color.RED );
        UtilDessin.PeindreTexteLimite("CONFIDENCES:", 100, iPositionMilieuZone+10, iY+(int)fHauteurCumuleeDroite, g2D);
        ...
        if ( (iNombreLignesBD - regle.m_iOccurrencesGauche) == 0)
            UtilDessin.PeindreTexteLimite("ind�termin� car 'non A' n'apparait jamais dans la BD.", 220, iPositionMilieuZone+100, iY+(int)fHauteurCumuleeDroite, g2D);
        else {
        ...
        if ( (iNombreLignesBD - regle.m_iOccurrencesDroite) == 0)
            UtilDessin.PeindreTexteLimite("ind�termin� car 'non B' n'apparait jamais dans la BD.", 220, iPositionMilieuZone+100, iY+(int)fHauteurCumuleeDroite, g2D);
        else {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if ( (m_tReglesFiltrees == null) || (m_iIndiceRegleAffichee<0) ) { 
            ...
            g2D.setFont(m_fontEnTete);
            g2D.setColor( new Color(255, 30, 50) );
            fHauteurCumulee += UtilDessin.PeindreTexteCentreLimite("No rule returned for the selected critera.", getWidth(), 0, (int)fHauteurCumulee, g2D);
    ...
    public void EnregistrerImageRegle(RegleAssociation regle, int iIndiceRegle, String sCheminFichier) {
