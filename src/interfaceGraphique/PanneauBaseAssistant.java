...
public class PanneauBaseAssistant extends PanneauBase {//set window background color, next previous button
    ...
    private JIndicateurEtape m_indicateurEtape = null;
    ...
    public void initBaseComponents() {
        ...
        m_indicateurEtape = new JIndicateurEtape(m_iNumeroEtape, m_sIntituleEtape);
        m_indicateurEtape.SpecifierFichierAide(m_sFichierAide);
        add(m_indicateurEtape);
       
        // previous/next button if visible:
        if (m_iPanneauPrecedent == FenetrePrincipale.PANNEAU_AUCUN) //if no previous panel, then there is no previous step button
            jButtonPrecedent.setVisible(false); 
        
        if (m_iPanneauSuivant == FenetrePrincipale.PANNEAU_AUCUN)
    ...
    private void RecalculerZoneControles() {
        ...
        if (m_iPanneauPrecedent != FenetrePrincipale.PANNEAU_AUCUN)
            m_zoneControles.height -= (iHauteurPanneau - jButtonPrecedent.getY());
        else if (m_iPanneauSuivant != FenetrePrincipale.PANNEAU_AUCUN)
